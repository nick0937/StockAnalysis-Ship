...
| 說了這些字 | 跑什麼 | 產出 |
|---|---|---|
| 日報／今日報告／收盤報告／完整報告／跑報告／`/tw-stock-8` | **整套**（下面「每日跑報告」7 步） | `REPO/YYYYMMDD/index.html` |
| **即時**／盤中／live／即時建議／現在能不能買 | **只跑 `python build_live.py`** | `REPO/live/index.html`（覆蓋） |

都沒明講又在盤中（09:00~13:30）→ 先問清楚再跑。判定規則見 [`../報告守則.md`](../報告守則.md) 第 3.0 節。
...
# ★ 最後一步：建立 commit（只 commit 不 push）——改用 PowerShell 執行，勿用 Git Bash：
#   cmd /c '"C:\Users\user\Desktop\StockAnalysis-Ship\建立Commit.bat" < NUL'
```

**commit 由流程代跑、push 一律不碰**：即時頁跑完後接著執行 repo 根目錄的「建立Commit.bat」（PowerShell：`cmd /c '"C:\Users\user\Desktop\StockAnalysis-Ship\建立Commit.bat" < NUL'`；⚠ Git Bash 因中文檔名 codepage 問題會失敗），它<b>只 commit 不 push</b>；要推上 GitHub Pages 得由使用者自行跑「發布到GitHub.bat」或 `git push`。詳見守則第 12／12A 節。
